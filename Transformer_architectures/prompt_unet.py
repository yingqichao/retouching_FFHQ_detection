...
class PromptGenBlock(nn.Module):
    def __init__(self, prompt_dim=128, prompt_len=5, prompt_size=96, lin_dim=192):
        ...
        self.SAM_mapping_layer = nn.Sequential(
            nn.Conv2d(320 + lin_dim, lin_dim, 1, 1, 0),
            nn.PReLU(),
            nn.Conv2d(lin_dim, prompt_dim, 1, 1, 0),
        )

    def forward(self, x, SAM_maps=None):
        B, C, H, W = x.shape
        emb = x.mean(dim=(-2, -1))
        ## B, len
        ...
        if SAM_maps is None:
            ...
            ## todo: use SAM for prompt
            ## B, 320, H, W
            SAM_maps = F.interpolate(SAM_maps, (H, W), mode="bilinear")
            ## B, p, H, W
            prompt_param = self.SAM_mapping_layer(SAM_maps)
...
class PromptIR(nn.Module):
    def __init__(self,
        ...
        self.output = nn.Conv2d(int(dim * 2 ** 1), out_channels, kernel_size=1, stride=1, padding=0, bias=bias)

    def forward(self, inp_img, noise_emb=None, SAM_maps=None):
        ## todo: encoders
        ## 1
        ...
        if self.decoder:
            dec3_param = self.prompt3(latent, SAM_maps=SAM_maps)
        ...
        if self.decoder:
            dec2_param = self.prompt2(out_dec_level3, SAM_maps=SAM_maps)
        ...
        if self.decoder:
            dec1_param = self.prompt1(out_dec_level2, SAM_maps=SAM_maps)
        ...
        return out_dec_level1, out_dec_level1 + inp_img


if __name__ == '__main__':
    model = PromptIR(decoder=True).cuda()
