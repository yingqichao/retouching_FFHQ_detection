...
class Columbia(AbstractDataset):
    """
    directory structure:
    NIST16
    │── img
    ...
    def __init__(self, transform_list, tamp_list: str, mode: str):
        """
...
if __name__ == '__main__':
    ...
    for s in imlist:
        ...
        mask_im[mask_im > 0] = 1
        mask_im = 1 - mask_im
        if im_im.shape[0] != mask_im.shape[0] or im_im.shape[1] != mask_im.shape[1] or mask_im.sum() * 2 > \
                mask_im.shape[0] * mask_im.shape[1]:
            print("Skip:", im, mask)
            continue
        new_imlist.append(s)

    with open("/root/DistillationForgertDetection/Distillation_model/Unet_with_MAE_Augmentation/datasets/NIST16.txt",
              "w") as f:
        f.write('\n'.join(new_imlist) + '\n')
