...
class convnextv1_plugin(ConvNeXt):
    def __init__(self, opt, img_size=512, num_classes:list=[1000,1000], embed_dims=[96, 192, 384, 768], num_heads=[8,8,8,8],
        ...
        super(convnextv1_plugin, self).__init__(depths=[3, 3, 9, 3], dims=embed_dims, num_classes=num_classes)
        self.opt = opt
    ...
    def forward(self, x):
        B = x.shape[0]
        x, x_hier = self.forward_features(x)
        hierarchical_tokens = []
        for i in range(4):
            x_proj = x_hier[i]
