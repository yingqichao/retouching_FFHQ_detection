...
class inception_plugin(GoogLeNet):
    def __init__(self, opt, img_size=512, num_classes:list=[1000,1000], embed_dims=[192,480,528,1024], num_heads=[8,8,8,8],
        ...
        super(inception_plugin, self).__init__(num_classes=num_classes)
        self.opt = opt
    ...
    def forward(self, x):
        ...
        return [output_trans] # [["眼","提","平","白"]] 调用大眼的token：output[0][0]
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        model = inception_plugin(opt={"skip_calibrate":True}, num_classes=[1000, 1000, 1000, 1000]).cuda()
    ...
    # model = inception_plugin(opt={"skip_calibrate":True})
