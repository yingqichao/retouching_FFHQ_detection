...
class ddpm_unet_plugin(Unet):
    def __init__(self, opt,
                 dim=64,
                 dim_mults=(1, 2, 4, 8, 16),
                 flash_attn=False,
                 full_attn=(False, False, False, False, True),
                 img_size=512, num_classes:list=[1000,1000], layers=[3,4,6,3], embed_dims=[128,256,512,1024],
        ...
        super(ddpm_unet_plugin, self).__init__(dim=dim, dim_mults=dim_mults, flash_attn=flash_attn, full_attn=full_attn)
    ...
    def forward(self,x, time=None, x_self_cond = None):
        ...
        x_decoded = self.Decoder(x, x_hier, h, t, r)
    ...
    def forward_classification(self,x, time=None, x_self_cond = None):
        B = x.shape[0]
        x, x_hier, h, t, r = self.forward_feature(x, time, x_self_cond)
...
if __name__ == '__main__':
    # inference(input=2)
    model = ddpm_unet_plugin(opt={'skip_calibrate':True}).cuda()
    output = model(torch.rand((1, 3, 512, 512)).cuda())
    print(output)
