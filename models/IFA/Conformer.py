...
from models.IFA.Conformer_hierarchical import LayerNorm2d
class ConvBlock(nn.Module):

    def __init__(self, inplanes, outplanes, stride=1, res_conv=False, act_layer=nn.GELU, groups=1,
                 norm_layer=partial(LayerNorm2d, eps=1e-6), drop_block=None, drop_path=None): # nn.BatchNorm2d
...
class Med_ConvBlock(nn.Module):
    ...
    def __init__(self, inplanes, act_layer=nn.GELU, groups=1, norm_layer=partial(LayerNorm2d, eps=1e-6),
...
class Conformer(nn.Module):
    ...
    def __init__(self, patch_size=16, in_chans=3, num_classes=[1000], base_channel=64, channel_ratio=4, num_med_block=0,
        ...
        self.trans_cls_head = nn.ModuleList([])
        for num_class in num_classes:
            self.trans_cls_head.append(nn.Linear(embed_dim, num_class))

        self.pooling = nn.AdaptiveAvgPool2d(1)
        # self.conv_cls_head = nn.Linear(int(256 * channel_ratio), num_classes)
        self.conv_cls_head = nn.ModuleList([])
        for num_class in num_classes:
            self.conv_cls_head.append(nn.Linear(int(256 * channel_ratio), num_class))

        # Stem stage: get the feature maps by conv block (copied form resnet.py)
        self.conv1 = nn.Conv2d(in_chans, 64, kernel_size=7, stride=2, padding=3, bias=False)  # 1 / 2 [112, 112]
        self.bn1 = LayerNorm2d(64) #nn.BatchNorm2d(64)
        self.act1 = nn.GELU()
    ...
    def forward(self, x):
        ...
        for idx, _ in enumerate(self.num_classes):
            conv_cls.append(self.conv_cls_head[idx](x_p))

        # trans classification
        x_t = self.trans_norm(x_t)
        tran_cls = []
        for idx, _ in enumerate(self.num_classes):
            tran_cls.append(self.trans_cls_head[idx](x_t[:, 0]))
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        model = Conformer(num_classes=[1000,1000,1000,1000]).cuda()
