    ...
    def forward(self, x, features_to_be_reused=None, relations_to_be_reused=None):
        ...
        if features_to_be_reused is not None:
            ...
            if (identity.size(1) - 1) == 100:
                ...
                feature_temp = torch.nn.functional.adaptive_avg_pool2d(feature_temp, (10, 10)).view(B, C, identity.size(
                    1) - 1).transpose(1, 2)
            else:
                feature_temp = torch.nn.functional.interpolate(feature_temp, (20, 20), mode='nearest')
                feature_temp = torch.nn.functional.adaptive_avg_pool2d(feature_temp, (14, 14)).view(B, C, identity.size(
                    1) - 1).transpose(1, 2)
