...
class CrossViT(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.sm_mlp_head = nn.ModuleList([])
        self.lg_mlp_head = nn.ModuleList([])
        for num_class in num_classes:
            self.sm_mlp_head.append(nn.Sequential(nn.LayerNorm(sm_dim), nn.Linear(sm_dim, num_class)))
            self.lg_mlp_head.append(nn.Sequential(nn.LayerNorm(lg_dim), nn.Linear(lg_dim, num_class)))
    ...
    def forward(self, img):
        ...
        for idx, _ in enumerate(self.num_classes):
            cls_sm.append(self.sm_mlp_head[idx](sm_cls))
            cls_lg.append(self.lg_mlp_head[idx](lg_cls))
        return [cls_sm, cls_lg]
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        nin, nout = 3, 1
        X = torch.randn(1,nin, 480,480).cuda()
        ...
        model = CrossViT(image_size=480,num_classes=[1000,1000,1000,1000]).cuda()
