...
class Conformer_hierarchical(nn.Module):
    ...
    def __init__(self, in_chans=3, num_classes=[6,3], num_heads=[8,8,8,8], qkv_bias=False, qk_scale=None,
        ...
        drop_tokens_per_layer = [1. / drop_tokens_per_layer,
                                 1. / (drop_tokens_per_layer / 4),
                                 1. / (drop_tokens_per_layer / 16),
                                 1.0
        ...
        self.num_tokens_after_grouping = [int(self.num_tokens_per_layer[i]*drop_tokens_per_layer[i])
        ...
        for i in range(self.num_stages):
            ...
            if i != self.num_stages - 1:
                ...
                # group_token = nn.Parameter(torch.zeros(1, self.num_tokens_after_grouping[i], embed_dims[i]))
                # trunc_normal_(group_token, std=.02)
                # setattr(self, f"group_token_{i}", group_token)
    ...
    def forward_features(self, x):
        ...
        for i in range(self.num_stages):
            ...
            if i!=self.num_stages-1:
                ...
                ## todo: grouping
                # group_token = position_group(group_token)
                positions = position_group_tokens.repeat(B, 1)
                group_token = group_embeddings(positions)
                x_proj, attention_map = project(x_proj, group_token)
