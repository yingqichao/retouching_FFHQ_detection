...
class meiyan_hallucinate(BaseModel):
    def __init__(self, opt, args, train_loader=None, val_loader=None):
        ...
        """
        super(meiyan_hallucinate, self).__init__(opt, args)
        ### todo: 定义恢复网络
        ...
        if self.opt['model_load_number'] > 0:
            self.reload(
                pretrain=f"{self.out_space_storage}/models/{self.opt['task_name']}/{self.opt['model_load_number']}_ViT.pth",
                network=self.detection_model,
                strict=True)
            self.reload(
                pretrain=f"{self.out_space_storage}/models/{self.opt['task_name']}/{self.opt['model_load_number']}_recovery.pth",
    ...
    def save(self, *, accuracy, iter_label):
        ...
        self.save_network(
            pretrain=f"{self.out_space_storage}/models/{self.opt['task_name']}/{accuracy}_{iter_label}_recovery.pth",
            network=self.detection_model,
        )
