...
class SinusoidalPosEmb(nn.Module):
    ...
    def forward(self, x):
        ...
        if x.ndim==1:
            ...
            emb = x.repeat(1,self.dim//8) * emb[None, :]
...
class Unet(nn.Module):
    def __init__(
        ...
        if self.use_SRM:
            ...
            self.init_conv = nn.Conv2d(input_channels, init_dim-3*3*self.use_SRM, 7, padding=3)
        ...
        self.time_embedding = nn.Embedding(4*4, fourier_dim)
        print("Using time embedding in unet")

        self.default_time_zero = torch.zeros((1,time_dim)).cuda()
                ...
                if self.apply_bottleneck:
                    for idx in range(2):
                        self.bottlenecks.append(nn.Sequential(
                            nn.Conv2d(dim_in, dim_in//2, 1, padding=0),
                            nn.SiLU(),
                            nn.Conv2d(dim_in//2, dim_in, 1, padding=0),
                        ))
    ...
    def forward_feature(self, x, time=None, x_self_cond = None):
        B = x.shape[0]
        ...
        if self.use_SRM:
            ...
            x = torch.cat([self.BayarConv2D_3x3(x), self.BayarConv2D_5x5(x), self.BayarConv2D_7x7(x), self.init_conv(x)], dim=1)
        else:
            x = self.init_conv(x)
        ...
        if time is not None:
            ...
            t = torch.cat([self.time_embedding(time[:,i]) for i in range(4)],dim=1)
        else:
            t = self.default_time_zero.repeat(B,1)
        ...
        return x, x_hier, h, t, r

    def forward(self, x, time=None, x_self_cond = None):
        B = x.shape[0]

        x, x_hier, h, t, r = self.forward_feature(x, time, x_self_cond)

        x = self.Decoder(x, x_hier, h, t, r)

        return x, x_hier

    def Decoder(self, x, x_hier, h, t, r):
        for idx, (block1, block2, attn, upsample) in enumerate(self.ups):
            if self.residual_connection:
                x = torch.cat((x, h.pop() if not self.apply_bottleneck else self.bottlenecks[idx*2](h.pop())), dim = 1)
            x = block1(x, t)

            if self.residual_connection:
                x = torch.cat((x, h.pop() if not self.apply_bottleneck else self.bottlenecks[idx*2+1](h.pop())), dim = 1)
            x = block2(x, t)
            x = attn(x) + x

            x = upsample(x)

        if self.residual_connection:    x = torch.cat((x, r), dim = 1)
